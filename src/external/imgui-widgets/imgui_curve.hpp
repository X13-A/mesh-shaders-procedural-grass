...
template<int DIM>
inline void spline(const float *key, int num, float t, float *v) {
...
inline int Curve(const char *label, const ImVec2 &size, const int maxpoints, ImVec2 *points, int *selection, const ImVec2 &rangeMin = ImVec2(0, 0), const ImVec2 &rangeMax = ImVec2(1, 1)) {
    const float MOUSE_DRAG_THRESHOLD = 0.01f;
    ...
    const float pointRadiusInPixels = 15.0f;
    ...
    if (hovered)
    {
        ...
        if (currentSelection == -1)
        {
            if (g.IO.MouseDoubleClicked[0] || IsMouseDragging(0, MOUSE_DRAG_THRESHOLD))
    ...
    const bool draggingPoint = IsMouseDragging(0, MOUSE_DRAG_THRESHOLD) && currentSelection != -1;
    ...
    if (!IsMouseDragging(0, MOUSE_DRAG_THRESHOLD) && GetActiveID() == id && selection && *selection != -1 && currentSelection == -1)
    {
        ClearActiveID();
    }
